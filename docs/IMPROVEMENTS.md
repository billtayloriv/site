...
- [ ] Remove any event on `work-the-room.html` whose date has passed (visible card and matching JSON-LD Event block).
...
- [ ] Add `lastmod` dates to `sitemap.xml` entries, updated whenever that page changes.
...
- [ ] Move events into a single data file so the event cards and Event JSON-LD come from one place (the `update-events` skill does this on first run).
